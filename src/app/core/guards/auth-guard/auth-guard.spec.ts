...
describe('authGuard', () => {
  ...
  it('should allow an authenticated user to open the protected route', async () => {
    ...
    expect(component).toBeInstanceOf(ProtectedTestPage);
  ...
  it('should preserve the requested URL as returnUrl', async () => {
    authenticatedState.set(false);
    ...
    expect(redirectedUrl.queryParams['returnUrl']).toBe('/protected');
  });

  it('should allow navigation after authentication changes', async () => {
    const harness = await RouterTestingHarness.create();
