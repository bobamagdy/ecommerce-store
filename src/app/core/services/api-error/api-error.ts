...
export class ApiErrorService {
  ...
  report(error: HttpErrorResponse): void {
    this.messageState.set(this.resolveMessage(error));
  }
  ...
  private readBackendMessage(errorBody: unknown): string | null {
    ...
    const possibleMessage = (errorBody as Record<string, unknown>)['message'];

    if (typeof possibleMessage !== 'string') {
      return null;
    }

    const trimmedMessage = possibleMessage.trim();
